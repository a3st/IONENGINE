...
uint32_t Window::Impl::get_width() const {

	return width;
}

uint32_t Window::Impl::get_height() const {

	return height;
}

void* Window::Impl::get_native_handle() const {

	return reinterpret_cast<void*>(hwnd);
}

void Window::Impl::set_label(std::string_view const label) {
...
std::queue<WindowEvent>& Window::Impl::get_queue_message() {
...
bool Window::Impl::is_cursor() const {

	return cursor;
}

void Window::Impl::show_cursor(bool const show) {
...
Window::Window(std::string_view const label, uint32_t const width, uint32_t const height, bool const fullscreen) :
	impl(std::unique_ptr<Impl, impl_deleter>(new Impl())) {

	impl->initialize(label, width, height, fullscreen);
}
...
void Window::set_label(std::string_view const label) {

	impl->set_label(label);
}

std::queue<WindowEvent>& Window::get_queue_message() {

	return impl->get_queue_message();
}

bool Window::is_cursor() const {

	return impl->is_cursor();
}

void Window::show_cursor(bool const show) {

	impl->show_cursor(show);
}