...
public:
    ...
    D3DDevice(const uint32 adapter_index) {
        ...
        m_cmd_queues[CommandBufferType::Graphics] = nullptr;
        m_cmd_queues[CommandBufferType::Copy] = nullptr;
        m_cmd_queues[CommandBufferType::Compute] = nullptr;

        D3D12_COMMAND_QUEUE_DESC queue_desc{};
        queue_desc.Priority = D3D12_COMMAND_QUEUE_PRIORITY_HIGH;
        queue_desc.Type = D3D12_COMMAND_LIST_TYPE_DIRECT;
        THROW_IF_FAILED(m_d3d12_device->CreateCommandQueue(&queue_desc, __uuidof(ID3D12CommandQueue), m_cmd_queues[CommandBufferType::Graphics].put_void()));

        queue_desc.Type = D3D12_COMMAND_LIST_TYPE_COPY;
        THROW_IF_FAILED(m_d3d12_device->CreateCommandQueue(&queue_desc, __uuidof(ID3D12CommandQueue), m_cmd_queues[CommandBufferType::Copy].put_void()));

        queue_desc.Type = D3D12_COMMAND_LIST_TYPE_COMPUTE;
        THROW_IF_FAILED(m_d3d12_device->CreateCommandQueue(&queue_desc, __uuidof(ID3D12CommandQueue), m_cmd_queues[CommandBufferType::Compute].put_void()));
    }

    void wait(const CommandBufferType type, Fence* fence, const uint64 value) override {
        THROW_IF_FAILED(m_cmd_queues[type]->Wait(static_cast<D3DFence*>(fence)->get_d3d12_fence(), value));
    }

    void signal(const CommandBufferType type, Fence* fence, const uint64 value) override {
        THROW_IF_FAILED(m_cmd_queues[type]->Signal(static_cast<D3DFence*>(fence)->get_d3d12_fence(), value));
    }
