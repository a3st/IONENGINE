...
public:
    ...
    std::unique_ptr<Shader> create_shader(const std::vector<byte>& shader_blob) override {
        return std::make_unique<D3DShader>(m_d3d12_device, shader_blob);
    }
    ...
    std::unique_ptr<Resource> create_resource(const ResourceDesc& resource_desc, const ResourceFlags flags) override {
        return std::make_unique<D3DResource>(m_d3d12_device, resource_desc, flags);
    }

    std::unique_ptr<Resource> create_resource(const SamplerDesc& sampler_desc) override {
        return std::make_unique<D3DResource>(m_d3d12_device, sampler_desc);
    }
