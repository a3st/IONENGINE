    ...
    D3DPipeline(winrt::com_ptr<ID3D12Device4>& device, const GraphicsPipelineDesc& pipeline_desc) 
        ...
        for(auto& stage : pipeline_desc.stages) {
            switch(stage.shader_type) {
                case ShaderType::Vertex: graphics_pipeline_desc.VS = static_cast<D3DShader&>(stage.shader.get()).get_d3d12_shader(); break;
                case ShaderType::Pixel: graphics_pipeline_desc.PS = static_cast<D3DShader&>(stage.shader.get()).get_d3d12_shader(); break;
                case ShaderType::Geometry: graphics_pipeline_desc.GS = static_cast<D3DShader&>(stage.shader.get()).get_d3d12_shader(); break;
                case ShaderType::Domain: graphics_pipeline_desc.DS = static_cast<D3DShader&>(stage.shader.get()).get_d3d12_shader(); break;
                case ShaderType::Hull: graphics_pipeline_desc.HS = static_cast<D3DShader&>(stage.shader.get()).get_d3d12_shader(); break;
            }
        }
