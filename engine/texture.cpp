...
namespace ionengine
{
    ...
    auto Texture::create(uint32_t const width, uint32_t const height, TextureFlags const flags) -> bool
    {
        if (flags == TextureFlags::RenderTarget)
        {
            texture = device->get_device().create_texture(
        ...
        else if (flags == TextureFlags::DepthStencil)
        {
            texture = device->get_device().create_texture(
        ...
        {
            texture = device->get_device().create_texture(width, height, 1, 1, rhi::TextureFormat::RGBA8_UNORM,
    ...
    auto Texture::get_texture() const -> core::ref_ptr<rhi::Texture>
    {
        return texture;
    }
