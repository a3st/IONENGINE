    ...
    methods: {
        updateTabSlots() {
            ...
            $(this.$el)
                ...
                .each((_, element) => {
                    if (
                        element.__vueParentComponent.proxy.$.type ==
                        TabliComponent
                    ) {
                        element.__vueParentComponent.proxy.$parent.$data.targets.push(
                            element.__vueParentComponent.proxy.$props.target
                        );
        ...
        setActiveTabByIndex(index) {
            ...
            target.closest("button.btn-tab").addClass("active");

            this.$emit("select", target.get(0));

            $(`#${target.get(0).__vueParentComponent.proxy.$props.target}`).css(
                "display",
                "flex"
            );
        },
