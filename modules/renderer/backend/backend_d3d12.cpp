...
struct Device::Impl {
    ...
    Impl(uint32_t const texture_size, uint32_t const buffer_size,
         uint32_t const sampler_size, uint32_t const descriptor_layout_size,
         uint32_t const pipeline_size, uint32_t const shader_size,
         uint32_t const render_pass_size, uint32_t const command_list_size);

    void initialize(uint32_t const adapter_index,
                    SwapchainDesc const& _swapchain_desc);
...
Device::Impl::Impl(uint32_t const texture_size, uint32_t const buffer_size,
...
void Device::Impl::initialize(uint32_t const adapter_index,
                              SwapchainDesc const& _swapchain_desc) {
    THROW_IF_FAILED(DxcCreateInstance(
        CLSID_DxcCompiler, __uuidof(IDxcCompiler3),
        reinterpret_cast<void**>(dxc_compiler.GetAddressOf())));
    THROW_IF_FAILED(
        DxcCreateInstance(CLSID_DxcUtils, __uuidof(IDxcUtils),
                          reinterpret_cast<void**>(dxc_utils.GetAddressOf())));
    ...
    THROW_IF_FAILED(
        CreateDXGIFactory2(factory_flags, __uuidof(IDXGIFactory4),
                           reinterpret_cast<void**>(factory.GetAddressOf())));
    THROW_IF_FAILED(
        factory->EnumAdapters1(adapter_index, adapter.GetAddressOf()));
    THROW_IF_FAILED(D3D12CreateDevice(
        adapter.Get(), D3D_FEATURE_LEVEL_12_0, __uuidof(ID3D12Device4),
    ...
    // Setup Info Queue. Only in debug engine build
    {
        ...
        info_queue_filter.AllowList.NumSeverities =
            static_cast<uint32_t>(allow_severities.size());
    ...
    THROW_IF_FAILED(d3d12::create_descriptor_pool(
        ...
        D3D12_DESCRIPTOR_HEAP_FLAG_NONE, 8096,
        cbv_srv_uav_pool.GetAddressOf()));

    // Initialize RTV Heap Pool for resources
    THROW_IF_FAILED(d3d12::create_descriptor_pool(
        device.Get(), D3D12_DESCRIPTOR_HEAP_TYPE_RTV,
        D3D12_DESCRIPTOR_HEAP_FLAG_NONE, 256, rtv_pool.GetAddressOf()));

    // Initialize DSV Heap Pool for resources
    THROW_IF_FAILED(d3d12::create_descriptor_pool(
        device.Get(), D3D12_DESCRIPTOR_HEAP_TYPE_DSV,
        D3D12_DESCRIPTOR_HEAP_FLAG_NONE, 12, dsv_pool.GetAddressOf()));

    // Initialize Sampler Heap Pool for resources
    THROW_IF_FAILED(d3d12::create_descriptor_pool(
        device.Get(), D3D12_DESCRIPTOR_HEAP_TYPE_SAMPLER,
        D3D12_DESCRIPTOR_HEAP_FLAG_NONE, 2048, sampler_pool.GetAddressOf()));
    ...
    // Create CBV_SRV_UAV Heap for binding in Descriptor Table
    {
        ...
        THROW_IF_FAILED(device->CreateDescriptorHeap(
            ...
            reinterpret_cast<void**>(cbv_srv_uav_heap.GetAddressOf())));
    }
    ...
    // Create Sampler Heap for binding in Descriptor Table
    {
        ...
        THROW_IF_FAILED(device->CreateDescriptorHeap(
            &descriptor_heap_desc, __uuidof(ID3D12DescriptorHeap),
            reinterpret_cast<void**>(sampler_heap.GetAddressOf())));
    }
    ...
    // Create Memory Allocator
    {
        ...
        THROW_IF_FAILED(D3D12MA::CreateAllocator(
            &allocator_desc, memory_allocator.GetAddressOf()));
    }
    ...
    // Get Swapchain resources and create RTV's
    {
        ...
        for (uint32_t i = 0; i < _swapchain_desc.buffer_count; ++i) {
            ...
            THROW_IF_FAILED(swapchain->GetBuffer(
                ...
                reinterpret_cast<void**>(texture.resource.GetAddressOf())));

            auto render_target_view_desc = D3D12_RENDER_TARGET_VIEW_DESC{};
            render_target_view_desc.Format = swapchain_desc.Format;
            render_target_view_desc.ViewDimension =
                D3D12_RTV_DIMENSION_TEXTURE2D;

            auto& cur_allocation =
                texture.descriptor_allocations[D3D12_DESCRIPTOR_HEAP_TYPE_RTV];
            THROW_IF_FAILED(rtv_pool->allocate(cur_allocation.GetAddressOf()));

            device->CreateRenderTargetView(texture.resource.Get(),
                                           &render_target_view_desc,
                                           cur_allocation->cpu_handle());

            swapchain_textures[i] = textures.push(std::move(texture));

            THROW_IF_FAILED(d3d12::create_descriptor_range(
                device.Get(), cbv_srv_uav_heap.Get(), i * 1024, 1024,
                cbv_srv_uav_ranges[i].GetAddressOf()));
            THROW_IF_FAILED(d3d12::create_descriptor_range(
                device.Get(), sampler_heap.Get(), i * 1024, 1024,
                sampler_ranges[i].GetAddressOf()));
        }
    }
}

void Device::Impl::deinitialize() {
    wait_for_idle(QueueFlags::Graphics | QueueFlags::Copy |
                  QueueFlags::Compute);
    CloseHandle(fence_event);
}

Handle<Texture> Device::Impl::create_texture(
    Dimension const dimension, uint32_t const width, uint32_t const height,
    uint16_t const mip_levels, uint16_t const array_layers, Format const format,
    TextureFlags const flags) {
    auto get_dimension_type =
        [&](Dimension const dimension) -> D3D12_RESOURCE_DIMENSION {
        switch (dimension) {
            case Dimension::_1D:
    ...
    THROW_IF_FAILED(memory_allocator->CreateResource(
        ...
        texture_data.memory_allocation.GetAddressOf(), __uuidof(ID3D12Resource),
    ...
    if (flags & TextureFlags::DepthStencil) {
        ...
        auto& cur_allocation =
            texture_data.descriptor_allocations[D3D12_DESCRIPTOR_HEAP_TYPE_DSV];
        THROW_IF_FAILED(dsv_pool->allocate(cur_allocation.GetAddressOf()));

        device->CreateDepthStencilView(texture_data.resource.Get(),
    ...
    if (flags & TextureFlags::RenderTarget) {
        ...
        switch (dimension) {
            ...
            case Dimension::Cube: {
                ...
                render_target_view_desc.Texture2DArray.MipSlice =
                    mip_levels - 1;
        ...
        auto& cur_allocation =
            texture_data.descriptor_allocations[D3D12_DESCRIPTOR_HEAP_TYPE_RTV];
        THROW_IF_FAILED(rtv_pool->allocate(cur_allocation.GetAddressOf()));

        device->CreateRenderTargetView(texture_data.resource.Get(),
                                       &render_target_view_desc,
                                       cur_allocation->cpu_handle());
    }
    if (flags & TextureFlags::UnorderedAccess) {
        ...
        switch (dimension) {
            case Dimension::_1D: {
                unordered_access_view_desc.ViewDimension =
        ...
        auto& cur_allocation =
            texture_data
                .descriptor_allocations[D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV];
        THROW_IF_FAILED(
            cbv_srv_uav_pool->allocate(cur_allocation.GetAddressOf()));

        device->CreateUnorderedAccessView(texture_data.resource.Get(), nullptr,
    ...
    if (flags & TextureFlags::ShaderResource) {
        ...
        shader_resource_view_desc.Shader4ComponentMapping =
        ...
        device->CreateShaderResourceView(texture_data.resource.Get(),
...
Handle<Buffer> Device::Impl::create_buffer(size_t const size,
                                           BufferFlags const flags,
                                           uint32_t const element_stride) {
    ...
    THROW_IF_FAILED(memory_allocator->CreateResource(
        &allocation_desc, &resource_desc, initial_state, nullptr,
        buffer_data.memory_allocation.GetAddressOf(), __uuidof(ID3D12Resource),
    ...
    if (flags & BufferFlags::ConstantBuffer) {
        ...
        constant_buffer_view_desc.BufferLocation =
            buffer_data.resource->GetGPUVirtualAddress();
        constant_buffer_view_desc.SizeInBytes = static_cast<uint32_t>(size);

        THROW_IF_FAILED(cbv_srv_uav_pool->allocate(
            buffer_data.descriptor_allocation.GetAddressOf()));

        device->CreateConstantBufferView(
            &constant_buffer_view_desc,
            buffer_data.descriptor_allocation->cpu_handle());
    }
    if (flags & BufferFlags::UnorderedAccess) {
        ...
        unordered_access_view_desc.Buffer.NumElements =
            static_cast<uint32_t>(size) / element_stride;
        ...
        device->CreateUnorderedAccessView(
            buffer_data.resource.Get(), nullptr, &unordered_access_view_desc,
            buffer_data.descriptor_allocation->cpu_handle());
    }
    if (flags & BufferFlags::ShaderResource) {
        ...
        shader_resource_view_desc.Buffer.NumElements =
            static_cast<uint32_t>(size) / element_stride;
        ...
        device->CreateShaderResourceView(
            buffer_data.resource.Get(), &shader_resource_view_desc,
            buffer_data.descriptor_allocation->cpu_handle());
    }
...
Handle<RenderPass> Device::Impl::create_render_pass(
    ...
    for (uint32_t i = 0; i < render_pass_data.render_target_count; ++i) {
        ...
        render_pass_data.render_target_descs[i].cpuDescriptor =
            texture_data.descriptor_allocations[D3D12_DESCRIPTOR_HEAP_TYPE_RTV]
                ->cpu_handle();
    }
    ...
    if (depth_stencil != InvalidHandle<Texture>()) {
        ...
        render_pass_data.depth_stencil_desc.cpuDescriptor =
            texture_data.descriptor_allocations[D3D12_DESCRIPTOR_HEAP_TYPE_DSV]
                ->cpu_handle();
...
Handle<Sampler> Device::Impl::create_sampler(Filter const filter,
                                             AddressMode const address_u,
                                             AddressMode const address_v,
                                             AddressMode const address_w,
                                             uint16_t const anisotropic,
    ...
    auto get_address_mode =
        [&](AddressMode const address_mode) -> D3D12_TEXTURE_ADDRESS_MODE {
        switch (address_mode) {
    ...
    THROW_IF_FAILED(sampler_pool->allocate(
        sampler_data.descriptor_allocation.GetAddressOf()));

    device->CreateSampler(&sampler_desc,
                          sampler_data.descriptor_allocation->cpu_handle());
...
Handle<Shader> Device::Impl::create_shader(std::string_view const source,
                                           ShaderFlags const flags) {
    auto get_shader_type =
        [&](ShaderFlags const shader_flags) -> const wchar_t* {
        switch (shader_flags) {
            case ShaderFlags::Vertex:
                return L"vs_6_0";
            case ShaderFlags::Pixel:
                return L"ps_6_0";
            case ShaderFlags::Compute:
                return L"cs_6_0";
            default:
                return L"vs_6_0";
        }
    ...
    THROW_IF_FAILED(dxc_utils->CreateBlob(source.data(),
                                          static_cast<uint32_t>(source.size()),
                                          CP_UTF8, source_blob.GetAddressOf()));
    ...
    THROW_IF_FAILED(dxc_compiler->Compile(
        &source_buffer, arguments.data(),
        static_cast<uint32_t>(arguments.size()), nullptr, __uuidof(IDxcResult),
    ...
    THROW_IF_FAILED(result->GetOutput(
        ...
        reinterpret_cast<void**>(error_blob.GetAddressOf()), nullptr));

    if (error_blob && error_blob->GetStringLength() > 0) {
        std::string_view error_message = std::string_view(
            error_blob->GetStringPointer(), error_blob->GetStringLength());
        throw lib::Exception(
    ...
    THROW_IF_FAILED(result->GetOutput(
        DXC_OUT_OBJECT, __uuidof(IDxcBlob),
        reinterpret_cast<void**>(shader_blob.GetAddressOf()), nullptr));

    auto shader_data = Shader{};
    shader_data.flags = flags;

    shader_data.data.resize(shader_blob->GetBufferSize());
    std::memcpy(shader_data.data.data(), shader_blob->GetBufferPointer(),
...
Handle<DescriptorLayout> Device::Impl::create_descriptor_layout(
    std::span<DescriptorLayoutBinding const> const bindings,
    bool const is_compute) {
    auto get_descriptor_range_type = [&](DescriptorType const descriptor_type)
    ...
    auto get_shader_visibility =
        [&](ShaderFlags const shader_flags) -> D3D12_SHADER_VISIBILITY {
        switch (shader_flags) {
            case ShaderFlags::Vertex:
            ...
            case ShaderFlags::Domain:
                return D3D12_SHADER_VISIBILITY_DOMAIN;
            case ShaderFlags::Hull:
                return D3D12_SHADER_VISIBILITY_HULL;
            case ShaderFlags::All:
    ...
    std::map<std::pair<D3D12_DESCRIPTOR_RANGE_TYPE, D3D12_SHADER_VISIBILITY>,
             uint32_t>
        ranges_index;
    ...
    for (auto& binding : bindings) {
        auto it = ranges_index.find({get_descriptor_range_type(binding.type),
                                     get_shader_visibility(binding.flags)});

        if (it != ranges_index.end()) {
            descriptor_layout_data.bindings.insert(
                {binding.index,
                 {it->second, descriptor_layout_data.ranges.at(it->second)
                                  .NumDescriptors}});
            ...
            descriptor_layout_data.bindings.insert(
                {binding.index,
                 {static_cast<uint32_t>(descriptor_layout_data.ranges.size()),
                  0}});
            ++range.NumDescriptors;

            ranges_index.insert(
                {{range.RangeType, get_shader_visibility(binding.flags)},
                 static_cast<uint32_t>(descriptor_layout_data.ranges.size())});
    ...
    for (size_t i = 0; i < parameters.size(); ++i) {
        parameters.at(i).DescriptorTable.pDescriptorRanges =
            &descriptor_layout_data.ranges.at(i);
    ...
    THROW_IF_FAILED(D3D12SerializeRootSignature(
        ...
        serialized_blob.GetAddressOf(), nullptr));

    THROW_IF_FAILED(device->CreateRootSignature(
        0, serialized_blob->GetBufferPointer(),
        serialized_blob->GetBufferSize(), __uuidof(ID3D12RootSignature),
        reinterpret_cast<void**>(
            descriptor_layout_data.root_signature.GetAddressOf())));

    return descriptor_layouts.push(std::move(descriptor_layout_data));
}

void Device::Impl::delete_descriptor_layout(
    Handle<DescriptorLayout> const& descriptor_layout) {
...
Handle<Pipeline> Device::Impl::create_pipeline(
    ...
    for (auto const& input : vertex_descs) {
        ...
        input_element_desc.InputSlotClass =
        ...
        pipeline_data.vertex_strides[input.slot] +=
            get_format_size(input.format);
    }

    pipeline_desc.InputLayout.pInputElementDescs = input_element_descs.data();
    pipeline_desc.InputLayout.NumElements =
        static_cast<uint32_t>(input_element_descs.size());
    ...
    _rasterizer_desc.ConservativeRaster =
    ...
    for (auto& shader : shaders) {
        ...
        switch (shader_data.flags) {
            case ShaderFlags::Vertex:
                pipeline_desc.VS = shader_bytecode;
                break;
            case ShaderFlags::Pixel:
                pipeline_desc.PS = shader_bytecode;
                break;
            case ShaderFlags::Geometry:
                pipeline_desc.GS = shader_bytecode;
                break;
            case ShaderFlags::Domain:
                pipeline_desc.DS = shader_bytecode;
                break;
            case ShaderFlags::Hull:
                pipeline_desc.HS = shader_bytecode;
                break;
        }
    }

    auto _depth_stencil_desc = D3D12_DEPTH_STENCIL_DESC{};
    _depth_stencil_desc.DepthFunc =
        get_comparison_func(depth_stencil_desc.depth_func);
    ...
    _depth_stencil_desc.DepthWriteMask = depth_stencil_desc.write_enable
    ...
    render_target_blend_desc.SrcBlendAlpha =
        get_blend(blend_desc.blend_src_alpha);
    render_target_blend_desc.DestBlendAlpha =
        get_blend(blend_desc.blend_dst_alpha);
    render_target_blend_desc.BlendOpAlpha =
        get_blend_func(blend_desc.blend_op_alpha);
    ...
    for (uint32_t i = 0; i < render_pass_data.render_target_count; ++i) {
        ...
        pipeline_desc.RTVFormats[i] =
            render_pass_data.render_target_descs[i]
                .BeginningAccess.Clear.ClearValue.Format;
    }

    // HOTFIX
    pipeline_desc.DSVFormat =
        render_pass_data.has_depth_stencil
            ? render_pass_data.depth_stencil_desc.DepthBeginningAccess.Clear
                  .ClearValue.Format
            : DXGI_FORMAT_UNKNOWN;
    ...
    if(cache_name.has_value()) {
        ...
        if(it != shader_cache_data.end()) {
            ...
            cached_pso_state.CachedBlobSizeInBytes =
...
Handle<Pipeline> Device::Impl::create_pipeline(
    Handle<DescriptorLayout> const& descriptor_layout,
    Handle<Shader> const& shader, Handle<CachePipeline> const& cache_pipeline) {
    auto pipeline_data = Pipeline{};
...
Handle<CommandList> Device::Impl::create_command_list(QueueFlags const flags,
                                                      bool const bundle) {
    auto get_command_list_type =
        [&](QueueFlags const queue_flags) -> D3D12_COMMAND_LIST_TYPE {
        switch (queue_flags) {
            case QueueFlags::Graphics:
                return D3D12_COMMAND_LIST_TYPE_DIRECT;
            case QueueFlags::Copy:
    ...
    if (bundle) {
        THROW_IF_FAILED(device->CreateCommandAllocator(
            D3D12_COMMAND_LIST_TYPE_BUNDLE, __uuidof(ID3D12CommandAllocator),
            reinterpret_cast<void**>(
                command_list_data.command_allocator.GetAddressOf())));

        THROW_IF_FAILED(device->CreateCommandList1(
            0, D3D12_COMMAND_LIST_TYPE_BUNDLE, D3D12_COMMAND_LIST_FLAG_NONE,
            __uuidof(ID3D12GraphicsCommandList4),
            reinterpret_cast<void**>(
                command_list_data.command_list.GetAddressOf())));

    } else {
        THROW_IF_FAILED(device->CreateCommandAllocator(
            get_command_list_type(flags), __uuidof(ID3D12CommandAllocator),
            reinterpret_cast<void**>(
                command_list_data.command_allocator.GetAddressOf())));

        THROW_IF_FAILED(device->CreateCommandList1(
            0, get_command_list_type(flags), D3D12_COMMAND_LIST_FLAG_NONE,
            __uuidof(ID3D12GraphicsCommandList4),
            reinterpret_cast<void**>(
                command_list_data.command_list.GetAddressOf())));
    }
...
void Device::Impl::delete_command_list(
    Handle<CommandList> const& command_list) {
    command_lists.erase(command_list);
}

uint8_t* Device::Impl::map_buffer_data(Handle<Buffer> const& buffer,
                                       uint64_t const offset) {
    auto& buffer_data = buffers[buffer];

    auto properties = D3D12_HEAP_PROPERTIES{};
    buffer_data.memory_allocation->GetResource()->GetHeapProperties(&properties,
                                                                    nullptr);
    ...
    if (properties.Type == D3D12_HEAP_TYPE_UPLOAD ||
        ...
        THROW_IF_FAILED(buffer_data.resource->Map(
            0, &range, reinterpret_cast<void**>(&bytes)));
        bytes += offset;
    } else {
...
size_t Device::Impl::get_memory_required_size(ResourceHandle const& resource) {
    ...
    auto resource_visitor = make_visitor(
        [&](Handle<Texture> const& texture) {
            ...
            device->GetCopyableFootprints(&resource_desc, 0,
                                          resource_desc.MipLevels, 0, nullptr,
...
void Device::Impl::present() { swapchain->Present(0, 0); }

void Device::Impl::recreate_swapchain(
    uint32_t const width, uint32_t const height,
    std::optional<SwapchainDesc> swapchain_desc) {
    ...
    THROW_IF_FAILED(swapchain->ResizeBuffers(
        _swapchain_desc.BufferCount, width, height, _swapchain_desc.Format, 0));
...
uint64_t Device::Impl::submit(
    ...
    for (auto& command_list : command_lists) {
        ...
        batches.push_back(reinterpret_cast<ID3D12CommandList* const>(
            command_list_data.command_list.Get()));
    }

    queues[get_queue_index(flags)]->ExecuteCommandLists(
        static_cast<uint32_t>(batches.size()), batches.data());

    uint64_t fence_value_result = fence_values[get_queue_index(flags)].load();
    ++fence_values[get_queue_index(flags)];

    THROW_IF_FAILED(queues[get_queue_index(flags)]->Signal(
        fences[get_queue_index(flags)].Get(), fence_value_result));

    return fence_value_result;
}

uint64_t Device::Impl::submit_after(
    std::span<Handle<CommandList> const> const command_lists,
    uint64_t fence_value, QueueFlags const flags) {
    auto get_queue_index = [&](QueueFlags const queue_flags) -> uint32_t {
        switch (queue_flags) {
            case QueueFlags::Graphics:
                return 0;
    ...
    THROW_IF_FAILED(queues[get_queue_index(flags)]->Wait(
        fences[get_queue_index(flags)].Get(), fence_value));
    queues[get_queue_index(flags)]->ExecuteCommandLists(
        static_cast<uint32_t>(batches.size()), batches.data());
...
void Device::Impl::wait(uint64_t const fence_value, QueueFlags const flags) {
    ...
    if (fences[get_queue_index(flags)]->GetCompletedValue() < fence_value) {
        THROW_IF_FAILED(fences[get_queue_index(flags)]->SetEventOnCompletion(
            fence_value, fence_event));
        WaitForSingleObjectEx(fence_event, INFINITE, FALSE);
    }
}

bool Device::Impl::is_completed(uint64_t const fence_value,
                                QueueFlags const flags) const {
    auto get_queue_index = [&](QueueFlags const queue_flags) -> uint32_t {
        switch (queue_flags) {
            case QueueFlags::Graphics:
                return 0;
...
void Device::Impl::wait_for_idle(QueueFlags const flags) {
    if (flags & QueueFlags::Graphics) {
        ...
        THROW_IF_FAILED(
            fences[0]->SetEventOnCompletion(fence_value, fence_event));
    ...
    if (flags & QueueFlags::Copy) {
        ...
        THROW_IF_FAILED(
            fences[1]->SetEventOnCompletion(fence_value, fence_event));
    ...
    if (flags & QueueFlags::Compute) {
        ...
        THROW_IF_FAILED(
            fences[2]->SetEventOnCompletion(fence_value, fence_event));
...
void Device::Impl::command_list_reset(CommandList& command_list) {
    ...
    THROW_IF_FAILED(command_list.command_list->Reset(
        command_list.command_allocator.Get(), nullptr));
    ...
    switch (command_list.command_list->GetType()) {
        ...
        case D3D12_COMMAND_LIST_TYPE_COMPUTE: {
            ...
            command_list.command_list->SetDescriptorHeaps(
                2, descriptor_heaps.data());
        } break;
    }
...
void Device::Impl::set_viewport(Handle<CommandList> const& command_list,
...
void Device::Impl::set_scissor(Handle<CommandList> const& command_list,
...
void Device::Impl::barrier(Handle<CommandList> const& command_list,
                           std::span<MemoryBarrierDesc const> const barriers) {
    auto& command_list_data = command_lists[command_list];
    ...
    auto get_memory_state =
        [&](MemoryState const memory_state) -> D3D12_RESOURCE_STATES {
        switch (memory_state) {
            case MemoryState::Common:
                return D3D12_RESOURCE_STATE_COMMON;
            case MemoryState::RenderTarget:
                return D3D12_RESOURCE_STATE_RENDER_TARGET;
            case MemoryState::Present:
                return D3D12_RESOURCE_STATE_PRESENT;
            case MemoryState::CopySource:
                return D3D12_RESOURCE_STATE_COPY_SOURCE;
            case MemoryState::CopyDest:
                return D3D12_RESOURCE_STATE_COPY_DEST;
            case MemoryState::ShaderRead:
                return D3D12_RESOURCE_STATE_ALL_SHADER_RESOURCE;
            case MemoryState::VertexConstantBufferRead:
                return D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER;
            case MemoryState::IndexBufferRead:
                return D3D12_RESOURCE_STATE_INDEX_BUFFER;
            case MemoryState::DepthWrite:
                return D3D12_RESOURCE_STATE_DEPTH_WRITE;
            case MemoryState::DepthRead:
                return D3D12_RESOURCE_STATE_DEPTH_READ;
            case MemoryState::GenericRead:
                return D3D12_RESOURCE_STATE_GENERIC_READ;
            case MemoryState::NonPixelShaderRead:
                return D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE;
            case MemoryState::UnorderedAccess:
    ...
    command_list_data.command_list->ResourceBarrier(
        static_cast<uint32_t>(resource_barriers.size()),
...
void Device::Impl::begin_render_pass(
    ...
    for (uint32_t i = 0; i < render_pass_data.render_target_count; ++i) {
        std::memcpy(render_pass_data.render_target_descs[i]
                        .BeginningAccess.Clear.ClearValue.Color,
                    &clear_colors[i], sizeof(lib::math::Color));
    }

    if (render_pass_data.has_depth_stencil) {
        render_pass_data.depth_stencil_desc.DepthBeginningAccess.Clear
            .ClearValue.DepthStencil.Depth = clear_depth;
        render_pass_data.depth_stencil_desc.StencilBeginningAccess.Clear
        ...
        command_list_data.command_list->BeginRenderPass(
            ...
            &render_pass_data.depth_stencil_desc, D3D12_RENDER_PASS_FLAG_NONE);
    } else {
        command_list_data.command_list->BeginRenderPass(
            render_pass_data.render_target_count,
            render_pass_data.render_target_descs.data(), nullptr,
...
void Device::Impl::execute_bundle(
    Handle<CommandList> const& command_list,
    Handle<CommandList> const& bundle_command_list) {
    auto& command_list_data = command_lists[command_list];
    ...
    command_list_data.command_list->ExecuteBundle(
        bundle_command_list_data.command_list.Get());
}

void Device::Impl::dispatch(Handle<CommandList> const& command_list,
                            uint32_t const thread_group_x,
    ...
    command_list_data.command_list->Dispatch(thread_group_x, thread_group_y,
                                             thread_group_z);
}

void Device::Impl::bind_pipeline(Handle<CommandList> const& command_list,
                                 Handle<Pipeline> const& pipeline) {
    auto& command_list_data = command_lists[command_list];
    ...
    command_list_data.command_list->SetPipelineState(
        pipeline_data.pipeline_state.Get());

    if (!pipeline_data.is_compute) {
        command_list_data.command_list->IASetPrimitiveTopology(
            D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
        command_list_data.command_list->SetGraphicsRootSignature(
            pipeline_data.root_signature.Get());
    } else {
        command_list_data.command_list->SetComputeRootSignature(
            pipeline_data.root_signature.Get());
    }
}

void Device::Impl::bind_resources(
    Handle<CommandList> const& command_list,
    Handle<DescriptorLayout> const& descriptor_layout,
    std::span<DescriptorWriteDesc const> const write_descs) {
    auto& command_list_data = command_lists[command_list];
    ...
    std::vector<ComPtr<d3d12::DescriptorAllocation>> allocations(
        descriptor_layout_data.ranges.size());
    ...
    for (size_t i = 0; i < allocations.size(); ++i) {
        switch (descriptor_layout_data.ranges.at(i).RangeType) {
            ...
            case D3D12_DESCRIPTOR_RANGE_TYPE_UAV: {
                THROW_IF_FAILED(cbv_srv_uav_ranges[swapchain_index]->allocate(
                    descriptor_layout_data.ranges.at(i),
                    allocations.at(i).GetAddressOf()));
            } break;
            case D3D12_DESCRIPTOR_RANGE_TYPE_SAMPLER: {
                THROW_IF_FAILED(sampler_ranges[swapchain_index]->allocate(
                    descriptor_layout_data.ranges.at(i),
                    allocations.at(i).GetAddressOf()));
            } break;
        }
        ...
        if (!descriptor_layout_data.is_compute) {
            command_list_data.command_list->SetGraphicsRootDescriptorTable(
                static_cast<uint32_t>(i), allocations.at(i)->gpu_handle());
        } else {
            command_list_data.command_list->SetComputeRootDescriptorTable(
                static_cast<uint32_t>(i), allocations.at(i)->gpu_handle());
        }
    }
    ...
    for (auto& write_desc : write_descs) {
        ...
        auto resource_visitor = make_visitor(
            [&](Handle<Texture> const& resource) {
                ...
                allocation = texture_data.descriptor_allocations
        ...
        auto const& binding_location =
            descriptor_layout_data.bindings.at(write_desc.index);

        uint32_t descriptor_size =
            device->GetDescriptorHandleIncrementSize(heap_type);

        auto cpu_handle =
            allocations[binding_location.range_index]->cpu_handle();
        cpu_handle.ptr += descriptor_size * binding_location.offset;

        device->CopyDescriptorsSimple(1, cpu_handle, allocation->cpu_handle(),
                                      heap_type);
    }
}

void Device::Impl::bind_vertex_buffer(Handle<CommandList> const& command_list,
    ...
    vertex_view.BufferLocation =
        buffer_data.resource->GetGPUVirtualAddress() + offset;
    vertex_view.SizeInBytes =
        static_cast<uint32_t>(buffer_data.resource->GetDesc().Width);
    vertex_view.StrideInBytes =
        command_list_data.binded_pipeline->vertex_strides[index];

    command_list_data.command_list->IASetVertexBuffers(index, 1, &vertex_view);
}

void Device::Impl::bind_index_buffer(Handle<CommandList> const& command_list,
                                     Handle<Buffer> const& buffer,
                                     uint64_t const offset) {
    auto& command_list_data = command_lists[command_list];
...
void Device::Impl::copy_texture_region(
    Handle<CommandList> const& command_list, Handle<Texture> const& dest,
    Handle<Buffer> const& source,
    std::span<TextureCopyRegion const> const regions) {
    auto& command_list_data = command_lists[command_list];
    ...
    auto result = std::min_element(regions.begin(), regions.end(),
                                   [&](auto const& lhs, auto const& rhs) {
                                       return lhs.mip_index < rhs.mip_index;
                                   });

    std::vector<D3D12_PLACED_SUBRESOURCE_FOOTPRINT> footprints(regions.size());

    device->GetCopyableFootprints(&resource_desc, result->mip_index,
                                  static_cast<uint32_t>(regions.size()), 0,
    ...
    for (auto const& region : regions) {
        ...
        command_list_data.command_list->CopyTextureRegion(
            &dest_location, 0, 0, 0, &source_location, nullptr);
    }
}

void Device::Impl::copy_buffer_region(Handle<CommandList> const& command_list,
                                      Handle<Buffer> const& dest,
                                      uint64_t const dest_offset,
                                      Handle<Buffer> const& source,
    ...
    command_list_data.command_list->CopyBufferRegion(
        dest_buffer.resource.Get(), dest_offset, source_buffer.resource.Get(),
        source_offset, size);
}

void Device::Impl::draw(Handle<CommandList> const& command_list,
    ...
    command_list_data.command_list->DrawInstanced(vertex_count, instance_count,
                                                  vertex_offset, 0);
}

void Device::Impl::draw_indexed(Handle<CommandList> const& command_list,
    ...
    command_list_data.command_list->DrawIndexedInstanced(
        index_count, instance_count, 0, 0, instance_offset);
}
...
AdapterDesc Device::Impl::adapter_desc() const {
    ...
    wcstombs_s(&result, reinterpret_cast<char*>(out_str.data()), length,
               reinterpret_cast<const wchar_t*>(adapter_desc.Description),
               length - 1);

    return AdapterDesc{.name = out_str,
...
Device::Device(uint32_t const adapter_index,
               SwapchainDesc const& swapchain_desc)
    : _impl(std::unique_ptr<Impl, impl_deleter>(
          new Impl(std::to_underlying(BackendLimits::Textures),
...
Device::~Device() { _impl->deinitialize(); }

Handle<Texture> Device::create_texture(
    Dimension const dimension, uint32_t const width, uint32_t const height,
    uint16_t const mip_levels, uint16_t const array_layers, Format const format,
    TextureFlags const flags) {
    return _impl->create_texture(dimension, width, height, mip_levels,
                                 array_layers, format, flags);
}
...
Handle<RenderPass> Device::create_render_pass(
    ...
    Handle<Texture> const& depth_stencil,
    std::optional<RenderPassDepthStencilDesc> const depth_stencil_desc) {
    return _impl->create_render_pass(colors, color_descs, depth_stencil,
                                     depth_stencil_desc);
}
...
Handle<Sampler> Device::create_sampler(Filter const filter,
                                       AddressMode const address_u,
                                       AddressMode const address_v,
                                       AddressMode const address_w,
                                       uint16_t const anisotropic,
                                       CompareOp const compare_op) {
    return _impl->create_sampler(filter, address_u, address_v, address_w,
                                 anisotropic, compare_op);
}
...
Handle<Shader> Device::create_shader(std::string_view const source,
                                     ShaderFlags const flags) {
...
Handle<DescriptorLayout> Device::create_descriptor_layout(
    std::span<DescriptorLayoutBinding const> const bindings,
    bool const is_compute) {
    return _impl->create_descriptor_layout(bindings, is_compute);
}

void Device::delete_descriptor_layout(
    Handle<DescriptorLayout> const& descriptor_layout) {
...
Handle<Pipeline> Device::create_pipeline(
    ...
    Handle<RenderPass> const& render_pass,
    Handle<CachePipeline> const& cache_pipeline) {
    return _impl->create_pipeline(descriptor_layout, vertex_descs, shaders,
                                  rasterizer_desc, depth_stencil_desc,
                                  blend_desc, render_pass, cache_pipeline);
}
...
uint8_t* Device::map_buffer_data(Handle<Buffer> const& buffer,
                                 uint64_t const offset) {
...
void Device::present() { _impl->present(); }
...
void Device::recreate_swapchain(uint32_t const width, uint32_t const height,
                                std::optional<SwapchainDesc> swapchain_desc) {
    _impl->recreate_swapchain(width, height, swapchain_desc);
}

uint64_t Device::submit(
    std::span<Handle<CommandList> const> const command_lists,
    QueueFlags const flags) {
    return _impl->submit(command_lists, flags);
}

uint64_t Device::submit_after(
    std::span<Handle<CommandList> const> const command_lists,
    uint64_t const fence_result, QueueFlags const flags) {
...
bool Device::is_completed(uint64_t const fence_result,
                          QueueFlags const flags) const {
...
void Device::set_viewport(Handle<CommandList> const& command_list,
                          int32_t const x, int32_t const y,
                          uint32_t const width, uint32_t const height) {
    _impl->set_viewport(command_list, x, y, width, height);
}

void Device::set_scissor(Handle<CommandList> const& command_list,
                         int32_t const left, int32_t const top,
                         int32_t const right, int32_t const bottom) {
    _impl->set_scissor(command_list, left, top, right, bottom);
}

void Device::barrier(Handle<CommandList> const& command_list,
                     std::span<MemoryBarrierDesc const> const barriers) {
...
void Device::begin_render_pass(
    ...
    std::span<lib::math::Color const> const clear_colors,
    float const clear_depth, uint8_t const clear_stencil) {
    _impl->begin_render_pass(command_list, render_pass, clear_colors,
...
void Device::execute_bundle(Handle<CommandList> const& command_list,
                            Handle<CommandList> const& bundle_command_list) {
    _impl->execute_bundle(command_list, bundle_command_list);
}

void Device::dispatch(Handle<CommandList> const& command_list,
                      uint32_t const thread_group_x,
                      uint32_t const thread_group_y,
                      uint32_t const thread_group_z) {
    _impl->dispatch(command_list, thread_group_x, thread_group_y,
                    thread_group_z);
}

void Device::bind_pipeline(Handle<CommandList> const& command_list,
                           Handle<Pipeline> const& pipeline) {
    _impl->bind_pipeline(command_list, pipeline);
}

void Device::bind_resources(
    Handle<CommandList> const& command_list,
    Handle<DescriptorLayout> const& descriptor_layout,
    std::span<DescriptorWriteDesc const> const write_descs) {
...
void Device::copy_buffer_region(Handle<CommandList> const& command_list,
                                Handle<Buffer> const& dest,
                                uint64_t const dest_offset,
                                Handle<Buffer> const& source,
                                uint64_t const source_offset,
                                size_t const size) {
    _impl->copy_buffer_region(command_list, dest, dest_offset, source,
                              source_offset, size);
}
...
void Device::bind_vertex_buffer(Handle<CommandList> const& command_list,
                                uint32_t const index,
                                Handle<Buffer> const& buffer,
                                uint64_t const offset) {
...
void Device::bind_index_buffer(Handle<CommandList> const& command_list,
                               Handle<Buffer> const& buffer,
                               uint64_t const offset) {
    _impl->bind_index_buffer(command_list, buffer, offset);
}

void Device::draw(Handle<CommandList> const& command_list,
                  uint32_t const vertex_count, uint32_t const instance_count,
                  uint32_t const vertex_offset) {
    _impl->draw(command_list, vertex_count, instance_count, vertex_offset);
}

void Device::draw_indexed(Handle<CommandList> const& command_list,
                          uint32_t const index_count,
                          uint32_t const instance_count,
                          uint32_t const instance_offset) {
    _impl->draw_indexed(command_list, index_count, instance_count,
                        instance_offset);
}

AdapterDesc Device::adapter_desc() const { return _impl->adapter_desc(); }