...
class Handle {
 public:
    ...
    Handle(Handle const& other)
        : _index(other._index), _generation(other._generation) {}
    ...
    bool operator==(Handle const& other) const {
        return std::tie(_index, _generation) ==
               std::tie(other._index, other._generation);
    }
...
class HandlePool {
 ...
 public:
    ...
    Handle<Type> push(Type const& element) {
        ...
        auto handle_data =
            HandleData{.data = element,
                       .generation = _data[index].generation +
    ...
    Handle<Type> push(Type&& element) {
        ...
        auto handle_data =
            HandleData{.data = std::move(element),
                       .generation = _data[index].generation +
                                     1 % std::numeric_limits<uint32_t>::max()};
    ...
    Type const& operator[](Handle<Type> const& handle) const {
        if (_data.at(handle.index()).generation != handle.generation()) {
            throw ionengine::lib::Exception(std::format(
                "Could not find required handle (id: {}, generation: {})",
