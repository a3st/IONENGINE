...
enum class MouseButton { Left, Right, Middle, Four, Five };

enum class InputState { Released, Pressed };
...
struct WindowEvent {
    std::variant<WindowEventData<WindowEventType::Closed>,
    ...
    static WindowEvent sized(uint32_t const width, uint32_t const height) {
        return WindowEvent{.data = WindowEventData<WindowEventType::Sized>{
    ...
    static WindowEvent keyboard_input(uint32_t const scan_code,
                                      InputState const input_state) {
        return WindowEvent{
            .data = WindowEventData<WindowEventType::KeyboardInput>{
                .scan_code = scan_code, .input_state = input_state}};
    }

    static WindowEvent mouse_input(MouseButton const button,
                                   InputState const input_state) {
        return WindowEvent{.data = WindowEventData<WindowEventType::MouseInput>{
                               .button = button, .input_state = input_state}};
    }

    static WindowEvent mouse_moved(int32_t const x, int32_t const y,
                                   int32_t const x_relative,
                                   int32_t const y_relative) {
        return WindowEvent{.data = WindowEventData<WindowEventType::MouseMoved>{
                               .x = x,
                               .y = y,
...
class Window {
 public:
    Window(std::string_view const label, uint32_t const width,
