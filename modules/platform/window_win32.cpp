...
struct Window::Impl {
    ...
    void initialize(std::string_view const label, uint32_t const width,
                    uint32_t const height, bool const fullscreen);
...
void Window::impl_deleter::operator()(Impl* ptr) const { delete ptr; }
...
void Window::Impl::initialize(std::string_view const label,
    ...
    hwnd = CreateWindow(wnd_class.lpszClassName,
    ...
    if (!RegisterRawInputDevices(raw_devices.data(),
                                 static_cast<uint32_t>(raw_devices.size()),
                                 sizeof(RAWINPUTDEVICE))) {
        throw std::runtime_error(
            "An error occurred while registering raw input devices");
    }
...
void Window::Impl::deinitialize() {
    ...
    RegisterRawInputDevices(raw_devices.data(),
                            static_cast<uint32_t>(raw_devices.size()),
                            sizeof(RAWINPUTDEVICE));
}

uint32_t Window::Impl::client_width() const { return _client_width; }

uint32_t Window::Impl::client_height() const { return _client_height; }
...
bool Window::Impl::cursor() const { return _cursor; }

void Window::Impl::cursor(bool const show) { _cursor = show; }

LRESULT Window::Impl::wnd_proc(HWND hWnd, UINT msg, WPARAM wParam,
                               LPARAM lParam) {
    auto window_impl =
    ...
    switch (msg) {
        ...
        case WM_SIZE: {
            ...
            window_impl->_client_width =
                std::max<uint32_t>(1, width - style_width);
            window_impl->_client_height =
                std::max<uint32_t>(1, height - style_height);

            window_impl->events.push(WindowEvent::sized(
                window_impl->_client_width, window_impl->_client_height));
        } break;
        ...
        case WM_MOUSEMOVE: {
            if (!window_impl->_cursor) {
                ...
                if (GetClientRect(hWnd, &rect)) {
                    auto point =
                        POINT{.x = rect.right / 2, .y = rect.bottom / 2};
        ...
        case WM_INPUT: {
            ...
            if (device_type == RIM_TYPEKEYBOARD) {
                ...
                switch (msg) {
                    case WM_KEYDOWN:
                    case WM_SYSKEYDOWN: {
                        window_event = WindowEvent::keyboard_input(
                            static_cast<uint32_t>(key), InputState::Pressed);
                    } break;
                    case WM_KEYUP:
                    case WM_SYSKEYUP: {
                        window_event = WindowEvent::keyboard_input(
                            static_cast<uint32_t>(key), InputState::Released);
                    } break;
                }
                ...
                switch (buttons) {
                    case RI_MOUSE_LEFT_BUTTON_DOWN: {
                        window_impl->events.push(WindowEvent::mouse_input(
                            MouseButton::Left, InputState::Pressed));
                    } break;
                    case RI_MOUSE_RIGHT_BUTTON_DOWN: {
                        window_impl->events.push(WindowEvent::mouse_input(
                            MouseButton::Right, InputState::Pressed));
                    } break;
                    case RI_MOUSE_MIDDLE_BUTTON_DOWN: {
                        window_impl->events.push(WindowEvent::mouse_input(
                            MouseButton::Middle, InputState::Pressed));
                    } break;
                    case RI_MOUSE_BUTTON_4_DOWN: {
                        window_impl->events.push(WindowEvent::mouse_input(
                            MouseButton::Four, InputState::Pressed));
                    } break;
                    case RI_MOUSE_BUTTON_5_DOWN: {
                        window_impl->events.push(WindowEvent::mouse_input(
                            MouseButton::Five, InputState::Pressed));
                    } break;
                    case RI_MOUSE_LEFT_BUTTON_UP: {
                        window_impl->events.push(WindowEvent::mouse_input(
                            MouseButton::Left, InputState::Released));
                    } break;
                    case RI_MOUSE_RIGHT_BUTTON_UP: {
                        window_impl->events.push(WindowEvent::mouse_input(
                            MouseButton::Right, InputState::Released));
                    } break;
                    case RI_MOUSE_MIDDLE_BUTTON_UP: {
                        window_impl->events.push(WindowEvent::mouse_input(
                            MouseButton::Middle, InputState::Released));
                    } break;
                    case RI_MOUSE_BUTTON_4_UP: {
                        window_impl->events.push(WindowEvent::mouse_input(
                            MouseButton::Four, InputState::Released));
                    } break;
                    case RI_MOUSE_BUTTON_5_UP: {
                        window_impl->events.push(WindowEvent::mouse_input(
                            MouseButton::Five, InputState::Released));
                    } break;
                }
                ...
                window_impl->events.push(WindowEvent::mouse_moved(
                    static_cast<int32_t>(cursor_point.x),
...
Window::Window(std::string_view const label, uint32_t const width,
               uint32_t const height, bool const fullscreen)
    : _impl(std::unique_ptr<Impl, impl_deleter>(new Impl())) {
    _impl->initialize(label, width, height, fullscreen);
}

Window::~Window() { _impl->deinitialize(); }

Window::Window(Window&& other) noexcept { _impl = std::move(other._impl); }
...
uint32_t Window::client_width() const { return _impl->client_width(); }

uint32_t Window::client_height() const { return _impl->client_height(); }

void* Window::native_handle() const { return _impl->native_handle(); }

void Window::label(std::string_view const label) { _impl->label(label); }

std::queue<WindowEvent>& Window::messages() { return _impl->messages(); }

bool Window::cursor() const { return _impl->cursor(); }

void Window::cursor(bool const show) { _impl->cursor(show); }
