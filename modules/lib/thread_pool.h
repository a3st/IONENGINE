...
enum class TaskPriorityFlags : uint8_t { High = 1 << 0, Low = 1 << 1 };
...
class ThreadPool {
 public:
    ...
    void push(TaskPriorityFlags const priority, Func&& func, Args&&... args) {
        ...
        _current_worker =
            (_current_worker + 1) % static_cast<uint16_t>(_workers.size());
    }

    inline size_t size() const;

    void wait_all(TaskPriorityFlags const flags = TaskPriorityFlags::High |
                                                  TaskPriorityFlags::Low);
...
extern std::optional<ThreadPool> _thread_pool;

#ifndef LIB_DECLARE_THREAD_POOL
#define LIB_DECLARE_THREAD_POOL() std::optional<lib::ThreadPool> lib::_thread_pool;
#endif

#ifndef LIB_INITIALIZE_THREAD_POOL_DEFAULT
#define LIB_INITIALIZE_THREAD_POOL_DEFAULT() lib::_thread_pool.emplace();
#endif

#ifndef LIB_INITIALIZE_THREAD_POOL
#define LIB_INITIALIZE_THREAD_POOL(ThreadCount) \
    lib::_thread_pool.emplace(ThreadCount);
#endif
