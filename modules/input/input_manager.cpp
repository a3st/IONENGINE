...
void InputManager::on_keyboard_event(
    platform::WindowEventData<platform::WindowEventType::KeyboardInput> const&
        event) {
    _current_keys.at(event.scan_code) = static_cast<bool>(event.input_state);
}

void InputManager::on_mouse_moved_event(
    platform::WindowEventData<platform::WindowEventType::MouseMoved> const&
        event) {
    _axis_values.at(static_cast<uint32_t>(AxisCode::MouseX)) =
        static_cast<float>(event.x_relative);
    _axis_values.at(static_cast<uint32_t>(AxisCode::MouseY)) =
        static_cast<float>(event.y_relative);
}

bool InputManager::key_down(KeyCode const key_code) const {
    return _current_keys[static_cast<uint32_t>(key_code)] &&
           !_previous_keys[static_cast<uint32_t>(key_code)];
}
