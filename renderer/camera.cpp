...
{
    Camera::Camera(rhi::Device& device, core::ref_ptr<rhi::MemoryAllocator> allocator,
                   CameraProjectionType const projection_type, uint32_t const resolution_width,
                   uint32_t const resolution_height)
        : device(&device), projection_type(projection_type), field_of_view(68.0f), near_dst(0.1f), far_dst(100.0f)
    {
    ...
    auto Camera::resize(rhi::Device& device, core::ref_ptr<rhi::MemoryAllocator> allocator, uint32_t const width,
    ...
    {
        // view = math::Matrixf::look_at_rh(position, position + rotation * math::Vector3f(0.0f, 0.0f, -1.0f),
