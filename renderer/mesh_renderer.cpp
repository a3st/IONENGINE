...
void MeshRenderer::update(float const delta_time, scene::Scene& scene, uint32_t const frame_index) {
...
void MeshRenderer::render(PipelineCache& pipeline_cache, ShaderCache& shader_cache, FrameGraph& frame_graph, uint32_t const frame_index) {
        ...
        [=, &pipeline_cache, &shader_cache](RenderPassContext const& context) {
            ...
            for(auto const& batch : _opaque_queue) {
                ...
                if(geometry_buffer.is_ok()) {
                    ...
                    _device->draw_indexed(context.command_list, geometry_buffer->index_size / sizeof(uint32_t), static_cast<uint32_t>(batch.instances.size()));
                }
            }
        }
    );
...
void MeshRenderer::apply_material(ShaderBinder& binder, ShaderProgram& shader_program, asset::Material& material, uint32_t const frame_index) {
        ...
        if(parameter.is_sampler2D()) {
            ...
            if(parameter.as_sampler2D().asset.is_ok()) {
                ...
                if(texture.is_ok()) {
                    ...
                    binder.update_resource(texture_location, texture->texture);
                    // the sampler position is always 1 greater than the texture position
                    binder.update_resource(texture_location + 1, texture->sampler);
    ...
    if(it != shader_program.uniforms.end()) {
        ...
        _upload_manager->upload_buffer_data(material_buffer, 0, std::span<uint8_t const>(reinterpret_cast<uint8_t const*>(material_buffer.data()), material_buffer.size()));
