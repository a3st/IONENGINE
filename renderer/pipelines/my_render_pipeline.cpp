...
{   
    builder
        .add_attachment(
            "ForwardColor",
            camera->get_render_target()->get_width(),
            camera->get_render_target()->get_height(),
            RGAttachmentUsage::RenderTarget,
        ...
        .add_attachment(
            ...
            camera->get_render_target()->get_width(),
            camera->get_render_target()->get_height(),
            RGAttachmentUsage::DepthStencil
        )
        .add_graphics_pass(
            "ForwardPass",
            camera->get_render_target()->get_width(),
            ...
            {},
            {
                /*RGAttachmentInfo { 
                    "ForwardColor", 
                    RGColorAttachment { 
                        rhi::RenderPassLoadOp::Clear, rhi::RenderPassStoreOp::Store,
            ...
            [&, test_shader, camera](RGRenderPassContext& ctx) {
                ...
                for(auto& task : tasks) {
                    ...
                    ctx.get_command_buffer().bind_vertex_buffer(task.drawable->vertex_buffer(), 0, task.drawable->vertex_buffer()->get_size());
                    ctx.get_command_buffer().bind_index_buffer(task.drawable->index_buffer(), 0, task.drawable->index_buffer()->get_size(), rhi::IndexFormat::Uint32);
