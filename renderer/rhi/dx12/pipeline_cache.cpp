...
auto ionengine::renderer::rhi::compare_op_to_d3d12(CompareOp const compare_op) -> D3D12_COMPARISON_FUNC {
...
auto ionengine::renderer::rhi::blend_to_d3d12(Blend const blend) -> D3D12_BLEND {
    ...
    root_signature(root_signature)
{
    ...
    HRESULT result = device->CreateGraphicsPipelineState(&d3d12_pipeline_desc, __uuidof(ID3D12PipelineState), pipeline_state.put_void());
    if(result == D3D12_ERROR_ADAPTER_NOT_FOUND | result == D3D12_ERROR_DRIVER_VERSION_MISMATCH | result == E_INVALIDARG) {
        d3d12_pipeline_desc.CachedPSO = {};
        THROW_IF_FAILED(device->CreateGraphicsPipelineState(&d3d12_pipeline_desc, __uuidof(ID3D12PipelineState), pipeline_state.put_void()));
    } else {
...
PipelineCache::PipelineCache(ID3D12Device4* device) : device(device) {
...

    {
        ...
        if(ifs.is_open()) {
            ...
            result = device->CreatePipelineLibrary(cache_bytes.data(), cache_bytes.size(), __uuidof(ID3D12PipelineLibrary), pipeline_library.put_void());
        }
        
        if(result == D3D12_ERROR_ADAPTER_NOT_FOUND | result == D3D12_ERROR_DRIVER_VERSION_MISMATCH | result == E_INVALIDARG) {
            THROW_IF_FAILED(device->CreatePipelineLibrary(nullptr, 0, __uuidof(ID3D12PipelineLibrary), pipeline_library.put_void()));
        } else {
            THROW_IF_FAILED(result);
        }
    }
...
) -> core::ref_ptr<Pipeline> 
{
    ...
    if(result != entries.end()) {
        ...
        auto pipeline = core::make_ref<Pipeline>(device, root_signature.get(), shader, rasterizer, blend_color, depth_stencil, nullptr);
