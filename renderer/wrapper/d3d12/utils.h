...
DXGI_FORMAT conv_format(const Format format) {
	...
	switch(format) {
		...
		default: assert(false && "passed invalid argument to conv_format"); return DXGI_FORMAT_UNKNOWN;
	}
}

D3D12_FILL_MODE conv_fill_mode(const FillMode fill_mode) {
	
	switch(fill_mode) {
		case FillMode::Solid: return D3D12_FILL_MODE_SOLID;
		case FillMode::Wireframe: return D3D12_FILL_MODE_WIREFRAME;
		default: assert(false && "passed invalid argument to conv_fill_mode"); return D3D12_FILL_MODE_SOLID;
	}
}

D3D12_CULL_MODE conv_cull_mode(const CullMode cull_mode) {
	
	switch(cull_mode) {
		case CullMode::None: return D3D12_CULL_MODE_NONE;
		case CullMode::Back: return D3D12_CULL_MODE_BACK;
		case CullMode::Front: return D3D12_CULL_MODE_FRONT;
		default: assert(false && "passed invalid argument to conv_cull_mode"); return D3D12_CULL_MODE_NONE;
	}
}

D3D12_COMPARISON_FUNC conv_comparison_func(const ComparisonFunc comparison_func) {
	...
	switch(comparison_func) {
		...
		default: assert(false && "passed invalid argument to conv_comparison_func"); return D3D12_COMPARISON_FUNC_ALWAYS;
	}
}

D3D12_STENCIL_OP conv_stencil_op(const StencilOp stencil_op) {
	...
	switch(stencil_op) {
		...
		default: assert(false && "passed invalid argument to conv_stencil_op"); return D3D12_STENCIL_OP_ZERO;
	}
}

D3D12_BLEND conv_blend(const Blend blend) {
	...
	switch(blend) {
		...
		default: assert(false && "passed invalid argument to conv_blend"); return D3D12_BLEND_ZERO;
	}
}

D3D12_BLEND_OP conv_blend_op(const BlendOp blend_op) {
	...
	switch(blend_op) {
		...
		default: assert(false && "passed invalid argument to conv_blend_op"); return static_cast<D3D12_BLEND_OP>(0);
	}
}

D3D12_SHADER_VISIBILITY conv_shader_visibility(const ShaderType shader_type) {
	...
	switch(shader_type) {
		...
		default: assert(false && "passed invalid argument to conv_shader_visibility"); return D3D12_SHADER_VISIBILITY_ALL;
	}
}

D3D12_DESCRIPTOR_RANGE_TYPE conv_descriptor_range_type(const ViewType view_type) {
	...
	switch(view_type) {
		...
		default: assert(false && "passed invalid argument to conv_descriptor_range_type"); return D3D12_DESCRIPTOR_RANGE_TYPE_SRV;
	}
}

D3D12_COMMAND_LIST_TYPE conv_command_buffer_type(const CommandBufferType cmd_list_type) {
	...
	switch(cmd_list_type) {
		...
		default: assert(false && "passed invalid argument to conv_command_buffer_type"); return D3D12_COMMAND_LIST_TYPE_DIRECT;
    }
}

D3D12_HEAP_TYPE conv_heap_type(const MemoryType memory_type) {
	...
	switch(memory_type) {
		...
		default: assert(false && "passed invalid argument to conv_heap_type"); return D3D12_HEAP_TYPE_DEFAULT;
    }
}
...
D3D12_RESOURCE_DIMENSION conv_resource_dimension(const ViewDimension view_dimension) {
	...
	switch(view_dimension) {
		...
		default: assert(false && "passed invalid argument to conv_resource_dimension"); return D3D12_RESOURCE_DIMENSION_UNKNOWN;
	}
}

D3D12_FILTER conv_filter(const Filter filter) {
	...
	switch(filter) {
		...
		default: assert(false && "passed invalid argument to conv_filter"); return D3D12_FILTER_ANISOTROPIC;
	}
}

D3D12_TEXTURE_ADDRESS_MODE conv_texture_address_mode(const TextureAddressMode address_mode) {
	...
	switch(address_mode) {
		...
		default: assert(false && "passed invalid argument to conv_texture_address_mode"); return D3D12_TEXTURE_ADDRESS_MODE_CLAMP;
	}
}

D3D12_DESCRIPTOR_HEAP_TYPE conv_descriptor_heap_type(const ViewType view_type) {
	...
	switch(view_type) {
		...
		default: assert(false && "passed invalid argument to conv_descriptor_heap_type"); return D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV;
	}
}

D3D12_RENDER_PASS_BEGINNING_ACCESS_TYPE conv_render_pass_type(const RenderPassLoadOp load_op) {
	...
	switch (load_op) {
		...
		default: assert(false && "passed invalid argument to conv_render_pass_type"); return D3D12_RENDER_PASS_BEGINNING_ACCESS_TYPE_DISCARD;
    }
}

D3D12_RENDER_PASS_ENDING_ACCESS_TYPE conv_render_pass_type(const RenderPassStoreOp store_op) {
	...
	switch (store_op) {
		...
		default: assert(false && "passed invalid argument to conv_render_pass_type"); return D3D12_RENDER_PASS_ENDING_ACCESS_TYPE_DISCARD;
    }
}
