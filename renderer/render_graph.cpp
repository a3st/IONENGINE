...
{
    RGResourcePool::RGResourcePool(rhi::Device& device)
        : device(&device),
          allocator(device.create_allocator(1024 * 1024, 32 * 1024 * 1024, rhi::MemoryAllocatorUsage::Default))
    {
    }

    auto RGResourcePool::get(rhi::TextureFormat const format, uint32_t const sample_count, uint32_t const width,
        ...
        -> core::ref_ptr<rhi::Texture>
    {
        ...
        else
        {
            ...
            if (entry != entries.end())
            {
                ...
                {
                    texture = device
                                  ->create_texture(allocator, width, height, 1, 1, format, rhi::TextureDimension::_2D,
                                                   flags, {})
                                  .get();
                ...
                auto allocation = ResourceAllocation{.chunk = entry->second.get(), .offset = offset};
            ...
            else
            {
                ...
                auto texture =
                    device
                        ->create_texture(allocator, width, height, 1, 1, format, rhi::TextureDimension::_2D, flags, {})
                        .get();
                ...
                auto allocation = ResourceAllocation{.chunk = chunk.get(), .offset = 0};
                allocations.emplace(hash, allocation);
    ...
    RGGraphicsRenderPass::RGGraphicsRenderPass(std::string_view const pass_name, uint32_t const width,
                                               uint32_t const height, std::vector<uint64_t> const& inputs,
                                               std::vector<RGGraphicsAttachment> const& outputs,
                                               graphics_pass_func_t callback)
        : RGRenderPass(pass_name), inputs(inputs), outputs(outputs), width(width), height(height), callback(callback)
    {
    }

    auto RGGraphicsRenderPass::setup(rhi::Device& device, std::unordered_map<uint64_t, RGAttachment>& attachments,
                                     RGResourcePool& resource_pool, BufferPool& buffer_pool,
                                     core::ref_ptr<rhi::Texture> swapchain, uint32_t const frame_index) -> void
    {
        ...
        for (auto& output : outputs)
        {
            ...
            if (result->second.usage == RGAttachmentUsage::RenderTarget)
            {
                ...
                else
                {
                    colors[offset].texture = resource_pool.get(
                        rhi::TextureFormat::RGBA8_UNORM, 1, result->second.width, result->second.height,
            ...
            else
            {
                ...
                depth_stencil =
                    rhi::RenderPassDepthStencilInfo{.depth_load_op = depth_stencil_attachment.depth_load_op,
                ...
                {
                    depth_stencil.value().texture =
                        resource_pool.get(rhi::TextureFormat::D32_FLOAT, 1, result->second.width, result->second.height,
                                          (rhi::TextureUsageFlags)(rhi::TextureUsage::DepthStencil), result->first);
                }
            }
        }
        ...
        std::array<core::ref_ptr<rhi::CommandBuffer>, 1> submits = {command_buffer};
        ...
        : device(&device), render_passes(std::move(render_passes)), attachments(std::move(attachments)), frame_index(0)
    {
        ...
        {
            auto frame_info = FrameInfo{.buffer_pool = core::make_ref<BufferPool>(device),
                                        .resource_pool = core::make_ref<RGResourcePool>(device)};
    ...
    auto RenderGraphBuilder::add_attachment(std::string_view const attachment_name, uint32_t const width,
                                            uint32_t const height, RGAttachmentUsage const usage,
                                            core::ref_ptr<RenderTarget> render_target) -> RenderGraphBuilder&
    {
        uint64_t const hash = XXHash64::hash(attachment_name.data(), attachment_name.size(), 0);
        auto attachment = RGAttachment{.name = std::string(attachment_name),
                                       .width = width,
                                       .height = height,
                                       .usage = usage,
    ...
    auto RenderGraphBuilder::add_graphics_pass(std::string_view const pass_name, uint32_t const width,
                                               uint32_t const height, std::vector<std::string> const& inputs,
                                               ...
                                               graphics_pass_func_t const& callback) -> RenderGraphBuilder&
    {
        ...
        {
            uint64_t const hash =
                XXHash64::hash(outputs[i].attachment_name.data(), outputs[i].attachment_name.size(), 0);
            auto attachment = std::make_pair(hash, outputs[i].data);
            output_attachments.emplace_back(attachment);
        }

        auto render_pass = std::make_unique<RGGraphicsRenderPass>(pass_name, width, height, input_attachments,
                                                                  output_attachments, callback);
        render_passes.emplace_back(std::move(render_pass));
        return *this;
    }

    auto RenderGraphBuilder::build(rhi::Device& device) -> core::ref_ptr<RenderGraph>
    {
        return core::make_ref<RenderGraph>(device, std::move(render_passes), std::move(attachments));
    }
} // namespace ionengine::renderer