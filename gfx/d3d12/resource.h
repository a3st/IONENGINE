...
template<>
class Resource<backend::d3d12> {
public:

    Resource(ID3D12Device4* d3d12_device, const MemoryType memory_type, const ResourceType resource_type, const ResourceDesc& resource_desc) 
        ...
        m_memory_ptr = MemoryAllocatorWrapper<backend::d3d12>::allocate(m_type, m_memory_type, resource_desc.width, 0, resource_desc.flags);
    ...
    Resource(ID3D12Device4* d3d12_device, const ResourceType resource_type, const winrt::com_ptr<ID3D12Resource>& resource, const ResourceFlags resource_flags) 
    ...
    Resource(ID3D12Device4* d3d12_device, const SamplerDesc& sampler_desc) : m_d3d12_device(d3d12_device), m_desc(sampler_desc) {
    ...
    ~Resource() {
        ...
        if(m_memory_ptr.heap) {
            ...
            MemoryAllocatorWrapper<backend::d3d12>::deallocate(m_type, m_memory_ptr, resource_desc.width, resource_desc.flags);
        }
    }
    ...
    const std::variant<SamplerDesc, ResourceDesc>& get_desc() const { return m_desc; }

    const std::variant<D3D12_SAMPLER_DESC, D3D12_RESOURCE_DESC>& get_d3d12_desc() const { return m_d3d12_desc; }
...
private:
    ...
    std::variant<D3D12_SAMPLER_DESC, D3D12_RESOURCE_DESC> m_d3d12_desc;

    ResourceType m_type;
    std::variant<SamplerDesc, ResourceDesc> m_desc;
