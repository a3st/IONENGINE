...
namespace ionengine::glb
{
    GLBModel::GLBModel(std::span<uint8_t> const data_bytes)
    {
        ...
        {
            auto data = AccessorData{.buffer_view = static_cast<uint32_t>(accessor["bufferView"].get_uint64().value()),
                                     .component_type =
                                         static_cast<ComponentType>(accessor["componentType"].get_uint64().value()),
        ...
        {
            auto data = BufferViewData{.buffer = static_cast<uint32_t>(buffer_view["buffer"].get_uint64().value()),
        ...
        for (auto mesh : document["meshes"])
        {
            ...
            for (auto primitive : mesh["primitives"])
            {
                ...
                {
                    auto attribute_data =
                        AttributeData{.attrib_name = std::string(attribute.unescaped_key().value()),
                                      .accessor = static_cast<uint32_t>(attribute.value().get_uint64().value())};
                    attributes.emplace_back(attribute_data);
                }

                auto primitive_data =
                    PrimitiveData{.attributes = std::move(attributes),
                                  ...
                                  .material = primitive.find_field("material").error() == simdjson::error_code::SUCCESS
                                                  ? static_cast<uint32_t>(primitive["material"].get_uint64().value())
                                                  : 0};
            ...
            auto mesh_data =
                MeshData{.name = std::string(mesh["name"].get_string().value()), .primitives = std::move(primitives)};
